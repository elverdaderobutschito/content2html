...
class Content2HTML_AjaxController {
    ...
    public function __construct() {
        ...
        add_action('wp_ajax_wpstatic_field_browser_posts', [$this, 'handleFieldBrowserPosts']);
        add_action('wp_ajax_wpstatic_field_browser_fields', [$this, 'handleFieldBrowserFields']);
    }
    ...
    public function handleFieldBrowserPosts(): void {
        ...
        foreach ($posts as $post) {
            $result[] = [
                'id' => $post->ID,
                'title' => get_the_title($post) ?: __('(no title)', 'content2html'),
    ...
    public function handleFieldBrowserFields(): void {
        ...
        if (!in_array($post->post_type, $settings['post_types'], true)) {
            wp_send_json_error(['message' => __('This post type is not enabled under Content.', 'content2html')]);
        }
